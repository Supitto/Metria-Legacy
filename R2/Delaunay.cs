...
namespace Metria.R2
{
	...
	{
		private List<Ponto> _pontos; // Grupo de pontos
		private List<Ponto> _fecho; // pontos do fecho
		...
		public void porIncremental()
		{
			int qtd = 0;//
			_fecho = new FechoConvexo(_pontos).MontaFecho(ref qtd);
		...
		{
			_pontos = pontos;// recebe os pontos
		...
		private List<ArestaDelaunay> encontraArestas(Ponto P, List<TrianguloDelaunay> morto)
		{
			...
			{
				if (T.Vizinho1 == null || T.Vizinho1.Circunscrito.PosicaoRelativa(P) != 2)// se não esta fora
				{
					if (T.Vizinho1 == null)
				...
				if(T.Vizinho2 == null || T.Vizinho2.Circunscrito.PosicaoRelativa( P)!=2)// se não esta fora
				{
					if (T.Vizinho2 == null)
